...
from PyQt5.QtWidgets import (
    ...
    QTableWidgetItem, QPushButton, QMessageBox, QFileDialog, QHeaderView,
    QAbstractItemView
)
...
class PageOperadores(QWidget, Form):
    ...
    def cargar(self):
        self._cache = self.dao.obtenerOperadores()
    ...
    def _nuevo_operador(self):
        ...
        if len(d["password"]) < 6:
            ...
            return
        exito, msg = self.dao.crearOperador(
            d["dni_nie"], d["nombre_completo"], d["email"], d["telefono"], d["password"]
        )
        if exito:
            self.dao.registrarActividad(
                self.usuario_actual.usuario_id, "Creación",
                f"Nuevo operador: {d['email']}", self._ip()
            )
    ...
    def _editar_operador(self, usuario):
        ...
        self.dao.registrarActividad(
            self.usuario_actual.usuario_id, "Modificación",
            f"Operador editado: {usuario.email}", self._ip()
        )
    ...
    def _exportar(self):
        ruta, _ = QFileDialog.getSaveFileName(self, "Exportar operadores", "operadores.csv", "CSV (*.csv)")
        if not ruta:
            return
        try:
