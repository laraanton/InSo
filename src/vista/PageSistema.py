...
class PageSistema(QWidget, Form):
    ...
    def _hacer_backup(self):
        ...
        exito, resultado = self.dao.hacerBackup(carpeta)
        ...
        if exito:
            ...
            self.dao.registrarActividad(
                self.usuario_actual.usuario_id, "Backup",
                f"Backup generado: {resultado}",
                self._ip()
            )
            self._poblar_tabla()
    ...
    def _abrir_carpeta(self, nombre_archivo):
        ...
        # Abre el explorador en la carpeta del backup (solo Windows)
        try:
